...
* [getDBSchema](#getdbschema) - Read-only table/column list for the agent's private DuckDB memory (agent_db).
* [getDBTablePreview](#getdbtablepreview) - A bounded row preview of one agent_db table (SELECT ... LIMIT n).
...
```typescript
import { TextqlCore } from "@textql/sdk/core.js";
import { agentsGetDBSchema } from "@textql/sdk/funcs/agents-get-db-schema.js";
...
async function run() {
  const res = await agentsGetDBSchema(textql, {
    body: {},
  });
  if (res.ok) {
    const { value: result } = res;
    ...
    console.log("agentsGetDBSchema failed:", res.error);
  }
}
...
```typescript
import { TextqlCore } from "@textql/sdk/core.js";
import { agentsGetDBTablePreview } from "@textql/sdk/funcs/agents-get-db-table-preview.js";
...
async function run() {
  const res = await agentsGetDBTablePreview(textql, {
    body: {},
  });
  if (res.ok) {
    const { value: result } = res;
    ...
    console.log("agentsGetDBTablePreview failed:", res.error);
  }
}
