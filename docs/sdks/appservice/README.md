...
* [appServiceGetAppDBSchema](#appservicegetappdbschema) - Server stream of live activity batches + presence snapshots, driven by  Valkey nudges over the app_activity:{app_id} channel; Postgres stays SSoT.
* [appServiceGetAppDBTablePreview](#appservicegetappdbtablepreview) - Presence heartbeat: sets a short-TTL Valkey key for the member and nudges  the app's stream. Presence never touches Postgres and never exposes emails.
...
async function run() {
  const result = await textql.appService.appServiceGetAppDBSchema({
    body: {},
  });
...
import { appServiceAppServiceGetAppDBSchema } from "@textql/sdk/funcs/app-service-app-service-get-app-db-schema.js";
...
async function run() {
  const res = await appServiceAppServiceGetAppDBSchema(textql, {
    body: {},
  });
  if (res.ok) {
    ...
    console.log("appServiceAppServiceGetAppDBSchema failed:", res.error);
  }
}
...
## appServiceGetAppDBTablePreview
...
async function run() {
  const result = await textql.appService.appServiceGetAppDBTablePreview({
    body: {},
  });
...
```typescript
import { TextqlCore } from "@textql/sdk/core.js";
import { appServiceAppServiceGetAppDBTablePreview } from "@textql/sdk/funcs/app-service-app-service-get-app-db-table-preview.js";
...
async function run() {
  const res = await appServiceAppServiceGetAppDBTablePreview(textql, {
    body: {},
  });
  if (res.ok) {
    const { value: result } = res;
    ...
    console.log("appServiceAppServiceGetAppDBTablePreview failed:", res.error);
  }
}
