...
* [rbacServiceSetRolePermissions](#rbacservicesetrolepermissions) - Bulk add/remove permissions on a role in one call, producing a single audit entry for the whole edit.

## rbacServiceSetRolePermissions
...
async function run() {
  const result = await textql.rbacService.rbacServiceSetRolePermissions({
    body: {},
  });
...
import { rbacServiceRBACServiceSetRolePermissions } from "@textql/sdk/funcs/rbac-service-rbac-service-set-role-permissions.js";
...
async function run() {
  const res = await rbacServiceRBACServiceSetRolePermissions(textql, {
    body: {},
  });
  if (res.ok) {
    ...
    console.log("rbacServiceRBACServiceSetRolePermissions failed:", res.error);
  }
}
