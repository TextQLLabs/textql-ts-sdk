"""FastAPI routes for TextQL agent chat, under /v3/textql.
...
from textql_sdk._connect.public import agent_pb2, chat_pb2, llm_model_pb2
...
_RESOLVERS = {
    ("ontology", "approve"): "approve_ontology_change",
    ("ontology", "reject"): "reject_ontology_change",
    ("context_prompt", "approve"): "approve_context_prompt_change",
    ("context_prompt", "reject"): "reject_context_prompt_change",
}
        ...
        while True:
            ...
            if event_type == "runError":
                logger.error(
                    ...
                    chat_id,
                    event.run_error.error,
                )
            else:
                logger.debug(
                    "[_watch_events] SSE %s | chat_id=%s #%d",
                    event_type,
                    chat_id,
                    event_count,
                )
    ...
    try:
        async with aclosing(events) as stream:
            async for event in stream:
                yield _sse(event)
    except Exception:  # noqa: BLE001 - SSE must report failures after headers are sent.
        # Headers have already been sent; report failures within the SSE protocol.
        yield _sse(
            {
                "type": "runError",
                "runError": {
                    "error": "TextQL stream failed. Check the server configuration and permissions."
                },
            }
        )
...
def _created_after(value: str | None) -> datetime | None:
    ...
    if value.startswith("since:"):
        try:
            day = datetime.strptime(value[len("since:") :], "%Y-%m-%d").replace(
                tzinfo=timezone.utc
            )
        except ValueError:
            return None
...
async def submit_questions(body: SubmitQuestionsRequest):
    ...
    request_type = (
        chat_pb2.SubmitQuestionsRequest
        if body.action == "submit"
        else chat_pb2.DismissQuestionsRequest
    )
    request = ParseDict(
        {
            "cellId": body.cellId,
            "answers": [
                answer.model_dump(exclude_none=True) for answer in body.answers
            ],
        },
        request_type(),
    )
    ...
    call = (
        chats.submit_questions if body.action == "submit" else chats.dismiss_questions
    )
    await call(request)
    return {"success": True}
...
async def get_config():
    ...
    try:
        ...
        if (
            not isinstance(me, ConnectError)
            and isinstance(me.email, str)
            and me.email.strip()
        ):
            email = me.email.strip()
    except Exception as err:  # noqa: BLE001 - Identity lookup is optional.
        logger.warning(
            "[get_config] WhoAmI failed; message attribution will fall back | %s", err
        )
    agent_id = _agent_id()
    response = await _get_streaming().agents.get_agent(
...
async def resolve_cell(
    ...
    """
    logger.info(
        "[resolve_cell] request | cell_id=%s kind=%s action=%s",
        cell_id,
        body.kind,
        body.action,
    )
    ...
    if method is None:
        raise HTTPException(
            status_code=400, detail=f"Cannot {body.action} a {body.kind} cell"
        )

    request_type = getattr(
        chat_pb2, "".join(part.title() for part in method.split("_")) + "Request"
    )
    await getattr(_get_streaming().chats, method)(request_type(cell_id=cell_id))
    return {"cell_id": cell_id, "kind": body.kind, "action": body.action}


@router.get("/chats", response_model=ListChatsResponse)
async def list_chats(
    search_term: str | None = Query(None, description="Filter chats by keyword"),
    q: str | None = Query(
        None, description="Same as search_term; what FilterToolbar sends"
    ),
    limit: int = Query(10, ge=1, le=100, description="Number of chats per page"),
    offset: int = Query(0, ge=0, description="Pagination offset"),
    page: int | None = Query(
        None, ge=0, description="Page index; overrides offset when set"
    ),
    page_size: int = Query(
        30, ge=1, le=100, description="Rows per page when using `page`"
    ),
    creator: list[str] = Query([], description="Creator member ids"),  # noqa: B008 - FastAPI parameter metadata.
    source: list[str] = Query([], description="ChatSource enum names"),  # noqa: B008 - FastAPI parameter metadata.
    scope: list[str] = Query([], description="'bookmarked' and/or 'shared'"),  # noqa: B008 - FastAPI parameter metadata.
    date: str | None = Query(None, description="Preset id or since:YYYY-MM-DD"),
    ...
    if offset > 2_147_483_647:
        raise HTTPException(400, "Pagination offset is too large.")
    ...
    known = [name for name in source if name in ChatSource.keys()]  # noqa: SIM118 - Protobuf enum, not a dictionary.
    if known:
    ...
    logger.info(
        "[list_chats] response | returned=%d total=%d has_more=%s",
        len(chats),
        total,
        has_more,
    )
...
async def list_chat_members():
    ...
    """
    resp = await _get_streaming().chats.get_members_with_chats(
        GetMembersWithChatsRequest()
    )
...
async def get_chat_history(
    ...
    limit: int | None = Query(
        ...
        ge=1,
        le=200,
        description="Cells per page; defaults to 50, or 200 with all_pages",
    ),
    skip: int = Query(
        0, ge=0, description="Number of cells to skip (pagination offset)"
    ),
    all_pages: bool = Query(
        False, description="If True, fetch all pages and return every cell"
    ),
):
    """Return the cell history for a chat, oldest-first.

    Cells use protobuf JSON, which is the same JSON the watch
    ...
    while True:
        page_num += 1
        resp = await _get_streaming().chats.get_chat_history(
            chat_pb2.HistoryRequest(chat_id=chat_id, limit=limit, skip=current_skip)
        )
        ...
        logger.debug(
            ...
            chat_id,
            len(cells),
            getattr(resp, "has_more", False),
        )
        all_cells.extend(MessageToDict(cell) for cell in cells)
    ...
    logger.info(
        "[get_chat_history] done | chat_id=%s total_cells=%d pages=%d",
        chat_id,
        len(all_cells),
        page_num,
    )
...
def _agent_id() -> str:
    return os.environ["TEXTQL_AGENT_ID"].strip()


async def _require_agent(chat_id: str):
    response = await _get_streaming().chats.get_chat(
        chat_pb2.GetChatRequest(chat_id=chat_id)
    )
    if not response.HasField("chat"):
        raise HTTPException(404, "Chat not found.")
    if response.chat.agent_id != _agent_id():
        raise HTTPException(
            409,
...
async def create_chat(body: dict | None = None):
    chats = _get_streaming().chats
    # The attached agent supplies model, tools, and connectors.
    request = ParseDict(
        {
            "model": "MODEL_UNKNOWN",
            "paradigm": {
        ...
        chat_pb2.CreateRequest(),
    )
    created = await chats.create_chat(request)
    chat_id = created.chat.id
    if not chat_id:
        raise HTTPException(502, "TextQL returned a chat without an ID.")
    try:
        attached = await chats.attach_agent_to_chat(
            chat_pb2.AttachAgentToChatRequest(chat_id=chat_id, agent_id=_agent_id())
        )
        if attached.chat.id != chat_id or attached.chat.agent_id != _agent_id():
            raise HTTPException(502, "TextQL did not confirm the agent attachment.")
    except Exception:  # noqa: BLE001 - Every failed attachment must clean up its chat.
        try:
            await chats.delete_chat(
                chat_pb2.DeleteChatRequest(chat_id=chat_id), timeout_ms=30_000
            )
        except Exception:  # noqa: BLE001 - Report incomplete cleanup for any failure.
            raise HTTPException(
                502,
...
async def attach_file(chat_id: str, request: Request, response: ApiResponse):
    ...
    async with request.form(max_files=1, max_fields=0) as form:
        ...
        if file is None or not hasattr(file, "read") or len(form.multi_items()) != 1:
            raise HTTPException(400, "Upload exactly one file using the file field.")
        content = await file.read(MAX_FILE_BYTES + 1)
        if len(content) > MAX_FILE_BYTES:
            raise HTTPException(413, "Files must be no larger than 20 MiB.")
        if not content:
...
async def send_message(
    ...
    await _require_agent(chat_id)
    sent = await _get_streaming().chats.send_message(
    ...
    logger.info(
        "[send_message] message accepted | chat_id=%s cell_id=%s", chat_id, sent.cell_id
    )
    ...
    logger.info(
        "[send_message] non-streaming mode ; collecting events | chat_id=%s", chat_id
    )
    ...
    final_cell_id = ""
    error_msg = ""
    async for data in _watch_events(
        chat_id, latest_cell_id=body.latest_cell_id, stop_on_run_complete=True
    ):
        if data["type"] == "cell":
        ...
        elif data["type"] in ("timeout", "streamEnded"):
            error_msg = "Stream ended before the run completed."
...
def _inject_base_href(html: str, document_url: str) -> str:
    ...
    allow-origin: *`.
    """
    base_tag = f'<base href="{escape(urljoin(document_url, "."), quote=True)}">'
...
async def preview_proxy(
    ...
    fit: str | None = Query(None, description="Pass 'chart' to inject the fit shim"),
):
    ...
    if 300 <= upstream.status_code < 400:
        ...
        raise HTTPException(502, "Preview redirects are not allowed.")
    ...
    if upstream.is_success and "text/html" in content_type:
        try:
            ...
            async for chunk in upstream.aiter_bytes():
                ...
                if len(content) > MAX_FILE_BYTES:
                    raise HTTPException(502, "Preview HTML is too large.")
            html = _inject_base_href(
                content.decode("utf-8", errors="replace"), str(upstream.url)
            )
        finally:
            await upstream.aclose()
        if fit == "chart":
