/** The browser half: markup, styles, and the script driving `<textql-app>`. Knows
 * nothing about keys or secrets — it only calls the routes `server.ts` mounts. */

/** One page for both views. `GET {apiBase}` is the SDK's list route. */
export function renderPage(apiBase: string): string {
	return `<!doctype html>
