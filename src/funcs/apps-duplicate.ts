...
export function appsDuplicate(
  ...
  Result<
    operations.AppServiceDuplicateAppResponse,
    | TextqlError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.AppServiceDuplicateAppRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/textql.rpc.public.app.AppService/DuplicateApp")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.AppServiceDuplicateAppResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
    M.json("default", operations.AppServiceDuplicateAppResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
