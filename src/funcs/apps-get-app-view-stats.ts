...
export function appsGetAppViewStats(
  ...
  Result<
    operations.AppServiceGetAppViewStatsResponse,
    | TextqlError
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        operations.AppServiceGetAppViewStatsRequest$outboundSchema,
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.AppServiceGetAppViewStatsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
    M.json(
      "default",
      operations.AppServiceGetAppViewStatsResponse$inboundSchema,
    ),
  )(response, req);
  if (!result.ok) {
