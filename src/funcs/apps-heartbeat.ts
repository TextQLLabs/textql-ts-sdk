...
export function appsHeartbeat(
  ...
  Result<
    operations.AppServiceAppHeartbeatResponse,
    | TextqlError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.AppServiceAppHeartbeatRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/textql.rpc.public.app.AppService/AppHeartbeat")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.AppServiceAppHeartbeatResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
    M.json("default", operations.AppServiceAppHeartbeatResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
