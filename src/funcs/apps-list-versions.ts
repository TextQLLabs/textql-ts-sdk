...
export function appsListVersions(
  ...
  Result<
    operations.AppServiceListAppVersionsResponse,
    | TextqlError
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        operations.AppServiceListAppVersionsRequest$outboundSchema,
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.AppServiceListAppVersionsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
    M.json(
      "default",
      operations.AppServiceListAppVersionsResponse$inboundSchema,
    ),
  )(response, req);
  if (!result.ok) {
