...
export function appsMoveAppToFolder(
  ...
  Result<
    operations.AppServiceMoveAppToFolderResponse,
    | TextqlError
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        operations.AppServiceMoveAppToFolderRequest$outboundSchema,
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.AppServiceMoveAppToFolderResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
    M.json(
      "default",
      operations.AppServiceMoveAppToFolderResponse$inboundSchema,
    ),
  )(response, req);
  if (!result.ok) {
