...
export function appsSetFavorite(
  ...
  Result<
    operations.AppServiceSetFavoriteResponse,
    | TextqlError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.AppServiceSetFavoriteRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/textql.rpc.public.app.AppService/SetFavorite")();
  ...
  const context = {
    ...
    operationID: "AppService_SetFavorite",
    oAuth2Scopes: null,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.AppServiceSetFavoriteResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
    M.json("default", operations.AppServiceSetFavoriteResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
