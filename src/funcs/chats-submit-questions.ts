...
export function chatsSubmitQuestions(
  ...
  Result<
    operations.ChatServiceSubmitQuestionsResponse,
    | TextqlError
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        operations.ChatServiceSubmitQuestionsRequest$outboundSchema,
        value,
      ),
  ...
  const path = pathToFunc(
    "/textql.rpc.public.chat.ChatService/SubmitQuestions",
  )();
  ...
  const context = {
    ...
    operationID: "ChatService_SubmitQuestions",
    oAuth2Scopes: null,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ChatServiceSubmitQuestionsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
    M.json(
      "default",
      operations.ChatServiceSubmitQuestionsResponse$inboundSchema,
    ),
  )(response, req);
  if (!result.ok) {
