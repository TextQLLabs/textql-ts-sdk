...
import { TextqlDefaultError } from "../models/errors/textql-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | TextqlDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | TextqlDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new TextqlDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new TextqlDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
